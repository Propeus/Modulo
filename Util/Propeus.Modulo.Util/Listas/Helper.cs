...
namespace Propeus.Modulo.Util.Listas
{
    ...
    public static partial class Helper
    {
        ...
        public static IEnumerable<T> Join<T>(this IEnumerable<T> esquerda, IEnumerable<T> direita)
        {
            if (esquerda.IsNull())
            {
                throw new ArgumentNullException(nameof(esquerda), Resources.ARGUMENTO_NULO);
            }

            if (direita.IsNull())
            {
                throw new ArgumentNullException(nameof(direita), Resources.ARGUMENTO_NULO);
            }

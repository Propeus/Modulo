...
namespace Propeus.Module.WatcherDynamicModule.Models
{
    internal class ModuleProviderInfo
    {
        ...
        public ModuleProviderInfo(string modulePath, bool isCurrentDomain, IAssemblyLoadContextContract assemblyLoadContextContract, IBuilderProxyContract builderProxyContract, List<string> listNameIgnoreModules)
        {
            Modules = new Dictionary<string, DynamicModuleInfo>();
        ...
        /// </summary>
        public Dictionary<string, DynamicModuleInfo> Modules { get; private set; }
        ...
        private void MapModules()
        {
            if (IsValidModule && (!IsLoaded || IsChanged))
            {
                ...
                Dictionary<string, DynamicModuleInfo> auxTypes = new Dictionary<string, DynamicModuleInfo>();
                foreach (Type type in types)
                {
                    ...
                    if (moduleAttr is not null)
                    {
                        ...
                        {
                            auxTypes[type.Name].ModuleName = type.Name;
                            auxTypes[type.Name].Module = new WeakReference<Type>(type);
                        }
                        else
                        {
                            auxTypes.Add(type.Name, new DynamicModuleInfo(type));
                        }
                    }
                    //ModuloDeExemploParaPropeusModuloDinamico
                    ...
                    if (moduleContractAttr is not null)
                    {
                        ...
                        {
                            var moduleInfo = new DynamicModuleInfo(moduleContractAttr.ModuleName);
                            moduleInfo.AddContract(type);

                            auxTypes.Add(moduleContractAttr.ModuleName, moduleInfo);
                        }
                    }
                }

                foreach (KeyValuePair<string, DynamicModuleInfo> lf in auxTypes)
                {
                    if (!Modules.ContainsKey(lf.Key) && lf.Value.Module.TryGetTarget(out Type? target))
                    {
                        //Gera o proxy para os novos modulos
                        _builderProxyContract.BuildModuleProxy(lf.Value.Module, lf.Value.Contracts, ref lf.Value._currentHash, lf.Value.ModuleProxy);
                    }
                }

                foreach (KeyValuePair<string, DynamicModuleInfo> rh in Modules)
                {
